...
import { Search, Clock, Filter, Bookmark, ExternalLink, Play, Pause, ChevronLeft, ChevronRight } from "lucide-react";
          ...
          {isSearching ? (
            ...
            >
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-lg bg-black/60 backdrop-blur-xl p-4 rounded-xl border border-white/10 text-center opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0">
                <p className="text-xs text-white/80 italic">"{selectedMoment.ocrText}"</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <footer className="h-32 bg-card/30 backdrop-blur-2xl border-t border-white/5 flex flex-col">
        <div className="flex-1 relative overflow-hidden">
          <ScrollArea className="w-full h-full">
            <div ref={timelineRef} className="flex items-end h-full px-[50vw] space-x-8 pb-6">
              {mockMoments.map((moment) => (
                ...
                >
                  ...
                  <div className={`timeline-marker ${selectedMoment.id === moment.id ? 'timeline-marker-active' : ''}`} />
              ...
              {[...Array(20)].map((_, i) => (
                <div key={`filler-${i}`} className="flex flex-col items-center space-y-2 opacity-20">
